...
| `fm-update.sh`           | Guarded canonical-source self-update of firstmate and local secondmate homes with existing remote-origin routing, reconciling redundant divergence and classifying every live mate left on the target commit for restart or fallback nudge |
...
| `fm-update-source-lib.sh` | Validated `config/update-source` resolver with a `kunchenguid/firstmate` default |
