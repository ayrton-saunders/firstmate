...
For a remote route, a read-only preflight must report the same canonical source before the configured code root or persistent home may move; the parent also verifies the reported commit before accepting the result.
