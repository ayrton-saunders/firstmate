...
For a remote route, the configured code root resolves the inherited canonical source on that host before the persistent home fast-forwards to the code-root commit; the parent verifies the reported commit before accepting the result.
