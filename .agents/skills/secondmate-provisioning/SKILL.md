...
`/updatefirstmate` is the one path that still follows that copy: it first updates the remote code root from the canonical source resolved on that host, verifies that the host reached the required commit, then syncs the home to that refreshed code-root commit.
