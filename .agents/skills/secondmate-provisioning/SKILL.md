...
That no-fetch path is a purely local fast-forward or redundant-divergence reconcile of tracked files, never a network fetch, and it never touches the gitignored operational dirs, so a secondmate's backlog, projects, and in-flight work are never disturbed; a linked worktree advances immediately, while a standalone clone that lacks the target receives firstmate updates through `/updatefirstmate`'s canonical-source refresh.
...
`/updatefirstmate` is the one path that still follows that copy: it first requires a read-only exact-source preflight, then updates the remote code root, verifies that the host reached the required commit, and syncs the home to that refreshed code-root commit. An older updater or mismatched source is left unchanged for one-time manual upgrade.
